...
{
    public CreatePrescriptionCommandValidator(IUnitOfWork unitOfWork)
    {
        ...
        RuleForEach(x => x.PrescriptionItems)
            .SetValidator(new CreatePrescriptionItemDtoValidator(unitOfWork));

        When(x => x.IsTransferred == true, () =>
        {
            RuleFor(x => x.Payments.Sum(a => a.Amount))
                .Equal(0)
                .WithMessage(Messages.AmountPaidMustZeroWhenTransferring);

            RuleFor(x => x.CashbackUsed)
                .Equal(0)
                .WithMessage(Messages.CashbackUsedMustZeroWhenTransferring);

            RuleFor(x => x.CreditUsed)
                .Equal(0)
                .WithMessage(Messages.CreditUsedMustZeroWhenTransferring);
